...
def run_model_with_filter(model=None, filter_type=None, *da_args, **model_kwargs): 
    ...
    for k in tqdm.trange(params["nt"]):
        ...
        if re.match(r"\AMPI_model\Z", parallel_flag, re.IGNORECASE):                                   
            ...
            # print(f"\nRank: {rank}, ensemble_vec shape after: {ensemble_vec.shape[1]}\n")

            # print(f"\nRank: {rank}, ensemble_vec shape: {ensemble_vec.shape[1]}\n")
        else:
        ...
        # print(f"[DEBUG] Updated ensemble shape: {np.array(ensemble_vec).shape}") # Debugging
        # stop the code
        # exit()
